...
import { getEvents } from "../event/events/store/eventsSlice"
import EventDetail from "../instructor/schedule/EventDetail"

const localizer = momentLocalizer(moment)

const index = () => {
  const dispatch = useDispatch()
  const { users } = useSelector((state) => state.events)
  ...
  useEffect(() => {
    if (users.length) {
      const myEventsList = users?.length
        ? users?.map((events) => {
            const start_time = moment
              .tz(events?.start_time, "Africa/Johannesburg")
              .format("DD/MM/YY HH:mm")
            const end_time = moment
              .tz(events?.end_time, "Africa/Johannesburg")
              .format("DD/MM/YY HH:mm")
            return {
              title: events?.event_name,
              start: moment(start_time, "DD/MM/YY HH:mm").toDate(),
              end: moment(end_time, "DD/MM/YY HH:mm").toDate(),
              allDay: false,
  ...
  const handleSelect = (e) => {
    let clickedEvent = users.filter((event) => event.event_code == e.desc)

    let obj = {
      "Event Name": clickedEvent[0].event_name,
      "Event Code": clickedEvent[0].event_code,
      "Event Category": clickedEvent[0].event_category.categorieName,
    ...
    <>
      ...
      <div className="card dashoboard-card">
        <div className="card-body m-2">
          <h2 className="mb-5">Event Schedule </h2>
          <Calendar
            localizer={localizer}
            events={eventsList}
            startAccessor="start"
